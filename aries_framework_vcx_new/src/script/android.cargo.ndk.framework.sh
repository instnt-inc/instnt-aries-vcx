...
# Required env vars
ARIES_VCX_ROOT_NEW=$(dirname $(dirname $(dirname $SCRIPT_DIR)))
ANDROID_BUILD_DEPS_DIR=${ARIES_VCX_ROOT_NEW}/target/android_build_deps
LANGUAGE="kotlin"
...
generate_bindings() {
    export FRAMEWORK_ROOT="${ARIES_VCX_ROOT_NEW}/aries_framework_vcx_new"
    export ANDROID_DEMO_DIR="${ARIES_VCX_ROOT_NEW}/aries/agents/MyUniffiAndroid"
    ...
    cp -R ${FRAMEWORK_ROOT}/src/org/hyperledger/ariesframeworkvcx/aries_framework_vcx_new.kt ${ANDROID_DEMO_DIR}/app/src/main/java/com/example/myuniffiandroid
...
build_uniffi_for_demo() {
    ...
    pushd ${FRAMEWORK_ROOT}
        ...
        cp ${ARIES_VCX_ROOT_NEW}/target/${TARGET}/debug/libaries_framework_vcx_new.so ${ABI_PATH}/libuniffi_aries_framework_vcx_new.so
